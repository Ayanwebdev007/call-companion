...
// Helper function to import leads
async function importLeads(sourceSheetIds, targetSheetId, businessId, userId) {
  try {
    const customers = await Customer.find({
      spreadsheet_id: { $in: sourceSheetIds },
      business_id: businessId
    }).lean();

    if (customers.length === 0) return;

    // Check availability of existing leads to avoid duplicates
    // We can check by 'original_id' if we store it, or by meta_lead_id in meta_data
    // For simplicity/performance, we might just check if a lead with same phone/email exists in target?
    // Or, if we are strictly importing *copies*, we should perhaps tag them with 'source_lead_id'.
    // Given the requirement "all old leads", we should try to copy them.

    const newCustomers = customers.map(c => {
      const { _id, ...rest } = c;
      return {
        ...rest,
        user_id: userId, // Assign to the user performing the action (or sheet owner?)
    ...
    await Customer.insertMany(newCustomers);
    console.log(`Imported ${newCustomers.length} leads to Unified Sheet ${targetSheetId}`);
